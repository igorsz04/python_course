...
servicesEmpty = []
i = 0
while i < len(servicesEmpty): #fragment kodu wykonywany warunkowo
    print(servicesEmpty[i])
    i+=1
else:
    print("...") #fragment kodu wykonywany zawsze

#FOR IN - oblicz srednia arytmetyczną 1000 losowych wartości z przedziału od 1 do 10
suma = 0
...
#przypisz 12 osob do 4 grup - w kazdej grupie wylosuj jednego lidera
...
while(isClosed == False):
    ...
    for key in products.keys():                             # pętla wypisująca zawartość słownika products gdy są na magazynie
        if(products[key][2] != 0):
            print("%3i | %20s | %5.2f | %3i | %15s" % (key,products[key][0],products[key][1],products[key][2],time.clock()))
    try:
